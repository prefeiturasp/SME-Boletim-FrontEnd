...
describe("LegendaCustomizada", () => {
  test("não renderiza nada se payload não for fornecido", () => {
    const { container } = render(<LegendaCustomizada />);
    expect(container.firstChild).toBeNull();
  });

  test("renderiza o título corretamente", () => {
    render(<LegendaCustomizada payload={[]} />);
    expect(
      screen.getByText(/Distribuição dos estudantes em cada nível/i)
    ).toBeInTheDocument();
  ...
  test("renderiza corretamente múltiplos itens", () => {
    ...
    render(<LegendaCustomizada payload={payload} />);
