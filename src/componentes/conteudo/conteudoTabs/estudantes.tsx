...
const toBooleanFlag = (valor: unknown): boolean => {
...
const normalizarEstudante = (item: Partial<EstudanteItem>): EstudanteItem => ({
  disciplina: item.disciplina || "",
  anoEscolar: item.anoEscolar || 0,
  turma: item.turma || "",
  alunoRa: item.alunoRa || 0,
  alunoNome: item.alunoNome || "",
  proficiencia: item.proficiencia || 0,
  nivelDescricao: item.nivelDescricao || "",
  sexoDescricao: item.sexoDescricao || null,
  raca: item.raca || null,
  pap: toBooleanFlag(item.pap),
  aee: toBooleanFlag(item.aee),
  possuiDeficiencia: toBooleanFlag(item.possuiDeficiencia),
});

const Estudantes: React.FC = () => {
  const [dados, setDados] = useState<EstudanteItem[]>([]);
  ...
  const buscarDadosEstudantes = async (paginaAtual = 1, tamanhoPagina = 10) => {
    try {
      ...
      const itensNormalizados: EstudanteItem[] = (
        resposta.estudantes.itens || []
      ).map((item: Partial<EstudanteItem>) => normalizarEstudante(item));

      setDados(itensNormalizados);
  ...
  const colunas = [
    ...
    { title: "EOL do estudante", dataIndex: "alunoRa", key: "alunoRa" },
    {
      ...
      dataIndex: "alunoNome",
      key: "alunoNome",
      render: (text: string, record: EstudanteItem) => (
        <span className="coluna-nome-estudante">
          <span>{text}</span>
          <span className="icones-indicadores">
            {record.pap === true && (
              <span title="PAP" aria-label="PAP">
                <LogoPAP />
              </span>
            )}
            {record.aee === true && (
              <span title="AEE" aria-label="AEE">
                <LogoAEE />
              </span>
            )}
            {record.possuiDeficiencia === true && (
              <span title="Acessibilidade" aria-label="Acessibilidade">
                <LogoAcessibilidade />
              </span>
            )}
          </span>
