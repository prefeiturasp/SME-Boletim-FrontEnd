...
describe("compararDadosSmeService", () => {
  describe("getAnosAplicacaoVisaoSme", () => {
    it("chama a API com a URL correta", async () => {
      ...
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/anos-aplicacao-sme",
      );
    ...
    it("propaga erro quando a API falha", async () => {
      ...
      await expect(getAnosAplicacaoVisaoSme()).rejects.toThrow("Erro na API");
  ...
  describe("getComponentesCurricularesVisaoSme", () => {
    it("chama a API com a URL correta incluindo anoAplicacao", async () => {
      ...
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/componentes-curriculares-sme/2025",
      );
      expect(mockServicos.get).toHaveBeenCalledTimes(1);
      expect(resultado).toEqual(mockResponse);
    });
    ...
    it("constrói URL corretamente com diferentes anos de aplicação", async () => {
      ...
      const testCases = [
        ...
        },
        {
          dreId: 200,
          anoAplicacao: 2024,
          expectedUrl: "/api/BoletimEscolar/componentes-curriculares-sme/2024",
        },
        {
          dreId: 300,
          anoAplicacao: 2023,
          expectedUrl: "/api/BoletimEscolar/componentes-curriculares-sme/2023",
        },
        {
          dreId: 400,
          anoAplicacao: 2022,
          expectedUrl: "/api/BoletimEscolar/componentes-curriculares-sme/2022",
        },
        {
          dreId: 500,
          anoAplicacao: 0,
          expectedUrl: "/api/BoletimEscolar/componentes-curriculares-sme/0",
        },
        {
          dreId: 600,
          anoAplicacao: -1,
          expectedUrl: "/api/BoletimEscolar/componentes-curriculares-sme/-1",
        },
      ];
    ...
    it("propaga erro quando a API falha", async () => {
      ...
      await expect(getComponentesCurricularesVisaoSme(2025)).rejects.toThrow(
    ...
    it("lida com diferentes tipos de anoAplicacao", async () => {
      ...
      const testCases = [
        ...
        },
        {
          anoAplicacao: 2025.5,
          expectedUrl:
            "/api/BoletimEscolar/componentes-curriculares-sme/2025.5",
        },
      ];
  ...
  describe("getAnosEscolaresUeVisaoSme", () => {
    it("chama a API com a URL correta incluindo anoAplicacao e disciplinaId", async () => {
      ...
      const resultado = await getAnosEscolaresUeVisaoSme(
        anoAplicacao,
        disciplinaId,
      );

      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/anos-escolares-sme/2025/1",
      );
      expect(mockServicos.get).toHaveBeenCalledTimes(1);
      expect(resultado).toEqual(mockResponse);
    });
    ...
    it("constrói URL corretamente com diferentes combinações de parâmetros", async () => {
      ...
      const testCases = [
        ...
        },
        {
          ...
          anoAplicacao: 2024,
          disciplinaId: 2,
          expectedUrl: "/api/BoletimEscolar/anos-escolares-sme/2024/2",
        },
        {
          dreId: 300,
          anoAplicacao: 2023,
          disciplinaId: 10,
          expectedUrl: "/api/BoletimEscolar/anos-escolares-sme/2023/10",
        },
        {
          dreId: 400,
          anoAplicacao: 0,
          disciplinaId: 0,
          expectedUrl: "/api/BoletimEscolar/anos-escolares-sme/0/0",
        },
        {
          dreId: 500,
          anoAplicacao: -1,
          disciplinaId: -5,
          expectedUrl: "/api/BoletimEscolar/anos-escolares-sme/-1/-5",
        },
      ];
      ...
      for (const testCase of testCases) {
        ...
        await getAnosEscolaresUeVisaoSme(
          testCase.anoAplicacao,
          testCase.disciplinaId,
        );
    ...
    it("propaga erro quando a API falha", async () => {
      ...
      await expect(getAnosEscolaresUeVisaoSme(2025, 1)).rejects.toThrow(
        "Erro 404 - Not Found",
      );
    ...
    it("funciona com números decimais na URL", async () => {
      ...
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/anos-escolares-sme/2025.7/1.3",
      );
      expect(resultado).toEqual(mockResponse);
    });
    ...
    it("lida com valores extremos", async () => {
      ...
      const testCases = [
        {
          anoAplicacao: Number.MAX_SAFE_INTEGER,
          disciplinaId: Number.MAX_SAFE_INTEGER,
        },
        {
          anoAplicacao: Number.MIN_SAFE_INTEGER,
          disciplinaId: Number.MIN_SAFE_INTEGER,
        },
  ...
  describe("Testes de integração e fluxo completo", () => {
    it("sequência típica de uso das funções", async () => {
      ...
      const anosEscolares = await getAnosEscolaresUeVisaoSme(
        anos[0].ano,
        componentes[0].id,
      );
      ...
      expect(mockServicos.get).toHaveBeenNthCalledWith(
        1,
        "/api/BoletimEscolar/anos-aplicacao-sme",
      );
      expect(mockServicos.get).toHaveBeenNthCalledWith(
  ...
  describe("Tratamento de erros específicos", () => {
    it.each([
      ...
      "lida com erro HTTP $status para todas as funções",
      async ({ status, message }) => {
        const erro = new Error(`${status} - ${message}`);

        // Testa getAnosAplicacaoVisaoSme
        mockServicos.get.mockRejectedValueOnce(erro);
        await expect(getAnosAplicacaoVisaoSme()).rejects.toThrow(
          `${status} - ${message}`,
        );
    ...
    it("lida com erro de rede", async () => {
      ...
      await expect(getAnosAplicacaoVisaoSme()).rejects.toThrow("Network Error");
  ...
  describe("Performance e edge cases", () => {
    it("funciona com valores NaN e Infinity", async () => {
      ...
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/componentes-curriculares-sme/NaN",
      );

      // @ts-ignore - Testando valores extremos
      await getAnosEscolaresUeVisaoSme(Infinity, -Infinity);
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/anos-escolares-sme/Infinity/-Infinity",
      );
    });
  ...
  describe("getDadosTabelaSME", () => {
    it("chama a URL correta com os três parâmetros", async () => {
      mockServicos.get.mockResolvedValueOnce({ dados: [] });
      await getDadosTabelaSME(2025, 5, 9);
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/dados-tabela-comparativa-sme/2025/5/9",
      );
    });

    it("retorna os dados da API", async () => {
      const mockData = { proficiencia: 250, nivel: "Adequado" };
      mockServicos.get.mockResolvedValueOnce(mockData);
      const resultado = await getDadosTabelaSME(2024, 4, 5);
      expect(resultado).toEqual(mockData);
    });

    it("propaga erro quando a API falha", async () => {
      mockServicos.get.mockRejectedValueOnce(new Error("Erro API"));
      await expect(getDadosTabelaSME(2025, 5, 9)).rejects.toThrow("Erro API");
    });
  });

  describe("getGraficoSME", () => {
    it("chama a URL correta com os três parâmetros", async () => {
      mockServicos.get.mockResolvedValueOnce([]);
      await getGraficoSME(2025, 5, 9);
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/comparacao-grafico-sme/2025/5/9",
      );
    });

    it("retorna os dados do gráfico", async () => {
      const mockData = [{ mes: "Jan", valor: 200 }];
      mockServicos.get.mockResolvedValueOnce(mockData);
      const resultado = await getGraficoSME(2024, 4, 5);
      expect(resultado).toEqual(mockData);
    });

    it("propaga erro quando a API falha", async () => {
      mockServicos.get.mockRejectedValueOnce(new Error("Erro API"));
      await expect(getGraficoSME(2025, 5, 9)).rejects.toThrow("Erro API");
    });
  });

  describe("getListaDres", () => {
    it("chama a URL correta com os três parâmetros", async () => {
      mockServicos.get.mockResolvedValueOnce([]);
      await getListaDres(2025, 5, 9);
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/dres-comparativo-sme/2025/5/9",
      );
    });

    it("retorna a lista de DREs", async () => {
      const mockData = [{ id: 1, nome: "DRE Leste" }];
      mockServicos.get.mockResolvedValueOnce(mockData);
      const resultado = await getListaDres(2024, 4, 5);
      expect(resultado).toEqual(mockData);
    });

    it("propaga erro quando a API falha", async () => {
      mockServicos.get.mockRejectedValueOnce(new Error("Erro API"));
      await expect(getListaDres(2025, 5, 9)).rejects.toThrow("Erro API");
    });
  });

  describe("getCardsDre", () => {
    it("inclui dreId na URL quando dreId > 0", async () => {
      mockServicos.get.mockResolvedValueOnce([]);
      await getCardsDre(10, 2025, 5, 9, 20);
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/cards-comparativo-sme-por-dre/2025/5/9?pagina=1&itensPorPagina=20&dreId=10",
      );
    });

    it("não inclui dreId na URL quando dreId === 0", async () => {
      mockServicos.get.mockResolvedValueOnce([]);
      await getCardsDre(0, 2025, 5, 9, 20);
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/cards-comparativo-sme-por-dre/2025/5/9?pagina=1&itensPorPagina=20",
      );
    });

    it("não inclui dreId na URL quando dreId < 0", async () => {
      mockServicos.get.mockResolvedValueOnce([]);
      await getCardsDre(-1, 2025, 5, 9, 50);
      expect(mockServicos.get).toHaveBeenCalledWith(
        "/api/BoletimEscolar/cards-comparativo-sme-por-dre/2025/5/9?pagina=1&itensPorPagina=50",
      );
    });

    it("retorna os cards da DRE", async () => {
      const mockData = [{ id: 1, nome: "UE Teste" }];
      mockServicos.get.mockResolvedValueOnce(mockData);
      const resultado = await getCardsDre(5, 2024, 4, 5, 10);
      expect(resultado).toEqual(mockData);
    });

    it("propaga erro quando a API falha", async () => {
      mockServicos.get.mockRejectedValueOnce(new Error("Erro API"));
      await expect(getCardsDre(1, 2025, 5, 9, 20)).rejects.toThrow("Erro API");
    });
  });
});
